...
class MockOBDGenerator {
  ...
  _tick() {
    ...
    const now = Date.now();
    const readings = [
      { pid: PIDS.SPEED.pid, value: Math.round(this.speed / 0.621371), timestamp: now }, // convert mph back to km/h for parser
      ...
      { pid: PIDS.BATTERY_VOLTAGE.pid, value: this.batteryV * 1000, timestamp: now }, // mV for parser
      { pid: PIDS.ODOMETER.pid, value: this.odometer * 10, timestamp: now }, // 10ths of km for parser
    ];
