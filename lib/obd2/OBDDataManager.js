...
class OBDDataManager {
  ...
  async _writeBleHeartbeat(snapshot) {
    ...
    try {
      ...
      // Only stamp obd_updated_at if we have at least one fresh OBD value —
      // otherwise the heartbeat is just relaying a location, not telemetry.
      let wroteOBD = false;
      if (snapshot.speed != null) { update.obd_speed = snapshot.speed; wroteOBD = true; }
      if (snapshot.rpm != null) { update.obd_rpm = snapshot.rpm; wroteOBD = true; }
      if (snapshot.fuelLevel != null) { update.obd_fuel = snapshot.fuelLevel; wroteOBD = true; }
      if (wroteOBD) update.obd_updated_at = new Date().toISOString();
