...
class OBDDataManager {
  constructor() {
    ...
    this.tripData = {
      ...
      hardAccelerations: 0,
      diagnosticCodes: [],
      // Time-weighted speed stats: each speed reading covers the time slice since
      // the previous reading, so avg/threshold math integrates over actual driving
      // time instead of being skewed by uneven polling intervals.
      movingTimeSeconds: 0,
      speedTimeProduct: 0,
      secondsOver80: 0,
      secondsOver90: 0,
  ...
  async _writeBleHeartbeat(snapshot) {
    ...
    try {
      ...
      const obdExtra = {};
      let wroteOBD = false;
      if (snapshot.speed != null) { obdExtra.obd_speed = snapshot.speed; wroteOBD = true; }
      if (snapshot.rpm != null) { obdExtra.obd_rpm = snapshot.rpm; wroteOBD = true; }
      if (snapshot.fuelLevel != null) { obdExtra.obd_fuel = snapshot.fuelLevel; wroteOBD = true; }
      ...
      // we can finally see (a) how often the BLE heartbeat actually writes
      // and (b) whether it's silently being rejected. Note: the lat/lng is
      // CACHED from the last fresh GPS write — fix_age cannot be measured
      // accurately here, so pass null and rely on app_state + source to
      // identify these in analytics.
      const { writeDriverLocation, LOCATION_SOURCES } = require('../locationWrite');
      ...
      await writeDriverLocation({
        ...
        source: LOCATION_SOURCES.BLE_HEARTBEAT,
        fixTimestamp: null,
  ...
  startRecording() {
    ...
    // DTC polling: initial poll 20s in (waits for ELM327 protocol lock), then
    // every 5 minutes for the rest of the trip. Catches intermittent faults
    // that wouldn't be visible from a single early poll. The _dtcNotified
    // flag in _notifyAdminsDTC keeps admins from getting more than one push
    // per trip even if every poll comes back with codes.
    const requestDTCs = () => {
    ...
    setTimeout(requestDTCs, 20000);
    this._dtcPollInterval = setInterval(() => {
      if (!this.isRecording) {
        clearInterval(this._dtcPollInterval);
        this._dtcPollInterval = null;
        return;
      }
      requestDTCs();
    }, 5 * 60 * 1000);
  ...
  _processReading(data) {
    ...
    switch (pid) {
      case PIDS.SPEED.pid: {
        ...
        // Time-weighted aggregates: use the slice from lastSpeedTime to now
        // (capped at 3s to avoid wild gaps from reconnects skewing avg).
        if (this.lastSpeedTime) {
          const timeDelta = (timestamp - this.lastSpeedTime) / 1000;
          if (timeDelta > 0 && timeDelta <= 3) {
            // Treat >5mph as "moving" so red-light idling doesn't drag avg down.
            if (this.lastSpeed > 5) {
              this.tripData.movingTimeSeconds += timeDelta;
              this.tripData.speedTimeProduct += this.lastSpeed * timeDelta;
            }
            if (this.lastSpeed > 80) this.tripData.secondsOver80 += timeDelta;
            if (this.lastSpeed > 90) this.tripData.secondsOver90 += timeDelta;

            // Hard brake/accel detection (existing logic)
            if (this.lastSpeed > 0) {
              const deceleration = (this.lastSpeed - speedMph) / timeDelta;
              if (deceleration > 7.5) this.tripData.hardBrakes++;
              const acceleration = (speedMph - this.lastSpeed) / timeDelta;
              if (acceleration > 7.5) this.tripData.hardAccelerations++;
            }
          }
        }
  ...
  getTripSummary() {
    ...
    const avgSpeed = this.tripData.movingTimeSeconds > 0
      ? Math.round(this.tripData.speedTimeProduct / this.tripData.movingTimeSeconds)
      : null;
    ...
    return {
      ...
      avg_speed: avgSpeed,
      seconds_over_80: Math.round(this.tripData.secondsOver80),
      seconds_over_90: Math.round(this.tripData.secondsOver90),
