...
class OBDBLEManager {
  ...
  async _initELM327() {
    ...
    await this._sendAndWait('ATAT1', 500);
    // Headers off
  ...
  _handleData(data) {
    console.log('[OBD-BLE] BLE DATA:', JSON.stringify(data));
  ...
  _parseLine(line) {
    console.log('[OBD-BLE] RAW LINE:', JSON.stringify(line));
    ...
    console.log('[OBD-BLE] HEX:', hex, 'starts41:', hex.startsWith('41'), 'len:', hex.length);
    ...
    // Mode 09 response (VIN) starts with '49'
    if (hex.startsWith('4902')) {
      // VIN is multi-frame, would need assembly — skip for now
      return;
    }
