...
class OBDBLEManager {
  ...
  // After 3 consecutive failures with the cached device ID, drop the ID and
  // do a fresh scan — the device ID may have changed (Veepeak power-cycled).
  _attemptReconnect(deviceId) {
    ...
    this.reconnectTimer = setTimeout(async () => {
      ...
      if (this._reconnectFailCount < 3) {
        ...
        // Cached ID isn't working — do a fresh scan
        console.log('[OBD-BLE] 3 failed reconnects — re-scanning for device');
        ...
        if (devices.length > 0) {
          ...
          // If still failing, future reconnects use the new device ID
