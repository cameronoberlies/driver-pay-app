...
// BleManager is loaded via require so old runtimes (1.0.7) without the
// native module don't crash at module load time. obdBLE.init() will return
// false on old runtimes and OBD features simply won't activate.
let BleManager = null;
try {
  BleManager = require('react-native-ble-plx').BleManager;
} catch (e) {
  console.log('[OBD-BLE] react-native-ble-plx not available on this runtime');
}
...
class OBDBLEManager {
  constructor() {
    ...
    this.driverId = null; // set by OBDDataManager.startRecording for log correlation
  ...
  async init() {
    // Native module not available on this runtime (old build)
      ...
      return new Promise((resolve) => {
        ...
        setTimeout(() => {
          ...
          logEvent(devices.length > 0 ? 'info' : 'warn', 'obd_scan_complete',
            ...
            { device_count: devices.length, devices: devices.map(d => ({ name: d.name, rssi: d.rssi })), driver_id: this.driverId });
          resolve(devices);
        }, timeoutMs);
      });
    } catch (e) {
  ...
  async connect(deviceId) {
    ...
    try {
      ...
      this.disconnectSubscription = this.bleManager.onDeviceDisconnected(deviceId, () => {
        ...
        logEvent('warn', 'obd_disconnected', 'OBD device disconnected unexpectedly', { device_id: deviceId, driver_id: this.driverId });
        // Clear OBD telemetry on driver_locations so the admin Live map shows
        // "no telemetry" instead of a frozen speed/rpm from before the drop.
        if (this.driverId) {
          try {
      ...
      logEvent('info', 'obd_connected', `OBD connected to ${device.name || deviceId}`,
        { device_id: deviceId, device_name: device.name, driver_id: this.driverId });
      ...
      logEvent('error', 'obd_connect_failed', `OBD connection failed: ${e.message}`,
        { device_id: deviceId, error: e.message, driver_id: this.driverId });
      this._setState(BLE_STATE.ERROR);
      return false;
    }
  }
  ...
  _parseLine(line) {
    ...
    if (!line || line.startsWith('AT') || line === 'OK' || line === 'SEARCHING...' ||
      ...
      // Track ECU error responses so we can diagnose "BLE healthy but PIDs
      // not flowing" cases (Gabrial 2026-05-15). Log only every Nth so we
      // don't flood system_logs when polling hits NO DATA every 2s.
      if (line === 'NO DATA' || line === 'STOPPED' || line === 'UNABLE TO CONNECT' || line === 'ERROR') {
        if (!this._ecuErrorCounts) this._ecuErrorCounts = {};
        this._ecuErrorCounts[line] = (this._ecuErrorCounts[line] || 0) + 1;
        // Log on the 1st, 10th, 50th occurrence — enough to triage without spam
        const count = this._ecuErrorCounts[line];
        if (count === 1 || count === 10 || count === 50 || count % 200 === 0) {
          logEvent('warn', 'obd_ecu_error',
            `ELM327 returned "${line}" (count: ${count})`,
            { response: line, count, driver_id: this.driverId });
        }
      }
      return;
    }
  ...
  // Strategy:
  //  - Attempts 1-3: retry cached device ID with 5s delay
  //  - Attempts 4-6: fresh scan with exponential backoff (15s, 30s, 60s)
  //  - After 6 total attempts: give up. Don't hammer BLE for hours.
  //    Reconnect will only retry on next trip start or app foreground event.
  _attemptReconnect(deviceId) {
    if (this.reconnectTimer) return;
    if (!this._reconnectFailCount) this._reconnectFailCount = 0;

    const MAX_RECONNECT_ATTEMPTS = 6;
    if (this._reconnectFailCount >= MAX_RECONNECT_ATTEMPTS) {
      console.log('[OBD-BLE] Max reconnect attempts reached — giving up');
      logEvent('warn', 'obd_reconnect_exhausted',
        'Stopped trying to reconnect after max attempts',
        { failed_attempts: this._reconnectFailCount, driver_id: this.driverId });
      return;
    }

    // Exponential backoff after the cached-ID phase.
    // Attempts 1-3 (cached): 5s, 5s, 5s
    // Attempts 4-6 (rescan): 15s, 30s, 60s
    let delayMs = 5000;
    if (this._reconnectFailCount >= 3) {
      const rescanAttempt = this._reconnectFailCount - 3; // 0, 1, 2
      delayMs = [15000, 30000, 60000][rescanAttempt] || 60000;
    }

    console.log(`[OBD-BLE] Attempting reconnect in ${delayMs / 1000}s (attempt ${this._reconnectFailCount + 1}/${MAX_RECONNECT_ATTEMPTS})...`);
    this.reconnectTimer = setTimeout(async () => {
      this.reconnectTimer = null;

      // Clear any lingering iOS-side connection state before retrying
      try { await this.bleManager.cancelDeviceConnection(deviceId); } catch {}

      let success = false;
      if (this._reconnectFailCount < 3) {
        // Phase 1: try cached device ID
        success = await this.connect(deviceId);
      } else {
        // Phase 2: cached ID isn't working — do a fresh scan
        console.log('[OBD-BLE] Rescanning for device');
        logEvent('warn', 'obd_rescanning', 'Reconnect failing, falling back to fresh scan',
          { failed_attempts: this._reconnectFailCount, original_device_id: deviceId, driver_id: this.driverId });
        const devices = await this.scan(8000);
        if (devices.length > 0) {
          devices.sort((a, b) => b.rssi - a.rssi);
          const newDeviceId = devices[0].id;
          success = await this.connect(newDeviceId);
          if (success) {
            this._reconnectFailCount = 0;
            return;
          }
