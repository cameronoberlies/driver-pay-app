  ...
  try {
    ...
    if (eventType === Location.GeofencingEventType.Enter) {
      ...
      if (activeTrip && !activeTrip.paused) {
        ...
        if (trip && trip.status === 'in_progress' && trip.designated_driver_id === userId) {
          // Only the designated driver can auto-end via geofence
          ...
          if (tripDuration >= MIN_TRIP_MINUTES) {
            ...
            // Capture OBD trip summary if recording — the geofence path was missing this
            // previously, so vehicle telemetry never got saved when trips auto-ended.
            let obdSummary = null;
            try {
              const { obdData, obdBLE } = require('./obd2');
              if (obdData?.isRecording) {
            ...
            // displays are OBD-only now.
            if (obdSummary) tripUpdate.obd_data = obdSummary;
            await client.from('trips').update(tripUpdate).eq('id', trip.id);
  ...
  async start() {
    try {
      ...
      if (fgStatus !== 'granted' || bgStatus !== 'granted') {
        ...
        return { success: false, reason: `permissions: fg=${fgStatus}, bg=${bgStatus}` };
      }
      ...
      await Location.startGeofencingAsync(GEOFENCE_TASK, [
        ...
        // Gas reminder geofence temporarily disabled — was causing registration failures
        // {
