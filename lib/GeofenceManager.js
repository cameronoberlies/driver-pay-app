...
const DEALERSHIP_COORDS = {
  ...
  radius: 300, // 300 meters (~1000 feet)
};
...
TaskManager.defineTask(GEOFENCE_TASK, async ({ data, error }) => {
  ...
  try {
    ...
    // ─── DEBOUNCE: Ignore duplicate events within 5 minutes ───────────────────────
    const eventKey = eventType === Location.GeofencingEventType.Exit ? 'exit' : 'enter';
    const debounceKey = `geofence_last_${eventKey}`;
    const lastEventTime = await AsyncStorage.getItem(debounceKey);
    if (lastEventTime && (Date.now() - parseInt(lastEventTime, 10)) < 5 * 60 * 1000) {
      console.log(`[Geofence] Debounced duplicate ${eventKey} event`);
      return;
    }
    ...
    if (eventType === Location.GeofencingEventType.Enter) {
      ...
      if (activeTrip && !activeTrip.paused) {
        ...
        if (trip && trip.status === 'in_progress') {
          // Only auto-end if trip has been in progress for at least 30 minutes
          const MIN_TRIP_MINUTES = 30;
