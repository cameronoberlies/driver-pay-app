...
export default function LiveFlightsScreen() {
  ...
  function groupFlightsByStatus() {
    return {
      ...
      'DELAYED': flights.filter(f => f.delay_minutes > 0 && f.status !== 'LANDED'),
    ...
    >
      ...
      {flights.length === 0 && (
        <View style={styles.emptyState}>
          ...
          <Text style={styles.emptySubtext}>No flights scheduled for today</Text>
        </View>
      )}
    </ScrollView>
  );
...
function FlightGroup({ status, flights, expanded, onToggle }) {
...
const styles = StyleSheet.create({
  ...
  groupTitle: { fontSize: 14, fontWeight: '700', color: '#fff', letterSpacing: 1 },
